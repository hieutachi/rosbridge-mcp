...
- **Topic data is untrusted input to the LLM.** A malicious or compromised rosbridge endpoint can embed prompt-injection payloads in the topic/service data this server returns, potentially steering the connected LLM into unintended tool calls — only connect to rosbridge servers you trust.
